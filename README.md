...
高级灰的克制 + 灰粉的温柔 · 柔中带刚 · **不像 AI** · 一看就是你的
...
3. （可选）打开 `brand-dna.md`，把默认品牌色替换成你自己的，并同步修改 `assets/template-*.html` 里 `:root` 的变量。注意：公众号模板（`template-wechat.html`）全部是内联样式，没有 CSS 变量，需要手动搜索替换色值。快捷方法：在所有模板文件中搜索 `#FECBCA` 替换为你的主色，`#403F44` 替换为你的强调色，`#FCF8DF` 替换为你的点缀色
