...
| 色名 | 色值 | 用途 |
|------|------|------|
| 灰粉色 | `#FECBCA` | 主色调、标题、超链接、重点标记、装饰色块 |
| 高级灰 | `#403F44` | 强调、正文重点、badges、连接线、深色面板 |
| 灰绿色 | `#FCF8DF` | 点缀、高亮下划线、CTA、标签、暖底点缀 |

三色比例原则：主色65% · 强调色25% · 点缀色10%（点缀色永远是点缀，不做主色）
...
- **高级灰的克制 + 灰粉的温柔** — 有品质、不花哨
- **灰粉暖意 · 高级灰力量感** — 柔中带刚、有呼吸感
- **不像AI** — 这是最高优先级的约束
...
| 类型 | 禁止 |
|------|------|
| 配色 | 蓝紫渐变、cyan、neon、纯黑白、AI常用的冷灰蓝调、任何多色渐变背景（占位色块用纯色 `#FFF8E1` 或 `#FECBCA`，荷光笔高亮的 `linear-gradient(transparent 60%, #FCF8DF 60%)` 保留） |
...
- **选中文本高亮**: `::selection { background: #FECBCA; color: #403F44; }`
